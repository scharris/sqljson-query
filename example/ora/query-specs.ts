import {QueryGroupSpec, QuerySpec, RecordCondition} from '../../src/query-specs';
   ...
   : QuerySpec
{
   return {
      ...
      tableJson: {
         ...
         fieldExpressions: [
            "id",
            { field: "name", jsonProperty: "genericName" },
            "mesh_id",
            "cid",
            "registered",
            "market_entry_date",
...
const queryGroupSpec: QueryGroupSpec = {
   ...
   querySpecs: [
      drugQuery("drugs query", { sql: "$$.name like :namePattern" }),
      drugQuery("drug for id query", { sql: "$$.cid = :cid" }),
   ]
};
