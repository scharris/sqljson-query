...
insert into compound(id, display_name, nctr_isis_id, cas, entered_by, entered, approved_by)
  select n,
    concat('Test Compound ',  n),
    concat('ISIS-', n),
    concat('5',n,n,n,n,'-',n,n),
    mod(n,2)+1,
    current_timestamp,
    mod(n+1,2) + 1
...
insert into drug(id, name, compound_id, category_code, descr, therapeutic_indications, mesh_id, cid, registered_by, registered, market_entry_date)
  select
    ...
    concat('This is drug number ', n, '.'),
    concat('Indication ', n),
    concat('MESH', n),
    n * 99,
    mod(n+1,2) + 1,
    current_timestamp,
    date_sub(current_date, interval (n * 100) day)
  from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into reference(id, publication)
 select 100 * n + 1, concat('Publication 1 about drug # ', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into reference(id, publication)
 select 100*n+ 2, concat('Publication 2 about drug # ', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into reference(id, publication)
 select 100*n + 3, concat('Publication 3 about drug # ', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;
...
insert into brand(drug_id, brand_name, language_code, manufacturer_id)
 select n, concat('Brand', n, '(TM)'), 'EN', mod(n,3)+1
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into advisory(id, drug_id, advisory_type_id, text)
 select 100*n+1, n, 1, concat('Advisory concerning drug ', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into advisory(id, drug_id, advisory_type_id, text)
 select 100*n+2, n, 2, concat('Caution concerning drug ', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;

insert into advisory(id, drug_id, advisory_type_id, text)
 select 123*n, n, 3, concat('Heard this might be bad -anon', n)
 from (select 1 as n union all select 2 union all select 3 union all select 4 union all select 5) n
;
